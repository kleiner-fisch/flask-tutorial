...
class Game_Controller:
    ...
    def make_claim(self, player_id, line_number):
        '''This method validates the received data and stores that a claim was made'''
        self.validate_make_claim(player_id, line_number)
        self.game.claim['player_id'] = player_id
        self.game.claim['line_number'] = line_number
    ...
    def reject_claim(self, player_id, counter_example):
        ...
        line_number = self.game.claim['line_number']        
        self.game.lines[line_number].won_by = player_id
    ...
    def accept_claim(self, player_id):
        ...
        if player_id != self.get_other_player(claiming_player):
         ...
         # in the validity test we already ensured that the counter example is valid, i.e. is stronger
        line_number = self.game.claim['line_number']        
        self.game.lines[line_number].won_by = claiming_player
        self.game.claim.clear()


    def check_winner(self, player_id):
        '''checks if the player has won, and if yes, records the winner in the game state'''
        won_lines =[self.game.lines[line_number].won_by == player_id for line_number in range(9)]
        won_5_lines = sum(won_lines) >= 5
        won_3_adjacent_lines = any([won_lines[line_number] and won_lines[line_number+1] and won_lines[line_number+2]
                                    for line_number in range(9 - 2)])
        return won_5_lines or won_3_adjacent_lines


    def validate_formation_complete(self, player_id, line_number, formation):
        is_muddy = self.game.lines[line_number].contains("MUD")
    ...
    def validate_make_claim(self, player_id, line_number):
        ...
        if not self.game.lines[line_number].is_open():
            self.game.winner = self.get_other_player(player_id)
            raise InvalidUserInputError("The claimed line already is closed.")     
        line_cards = self.game.lines[line_number].sides[player_id]
        formation = [c for c in line_cards if c in JOKERS + NUMBERS_CARDS]
        self.validate_formation_complete(player_id, line_number, formation)

    def turn_done(self, pid):
        '''finishes the turn for this player and makes it the next players turn'''
        # TODO should be validated, i.e. no open scout, no open claims etc.
        if self.game.current_player != pid:
            self.game.winner = self.get_other_player(pid)
            raise InvalidUserInputError("You can only end your turn on your turn.")        
        ...
        if self.check_winner(other_pid): 
            ...
            return
        self.game.current_player = other_pid
    ...
    def validate_reject_claim(self, player_id, counter_example):
        ...
        self.validate_counter_example_format(player_id, line_number, counter_example)
        self.validate_formation_complete(player_id, line_number, counter_example)
        self.validate_counter_example_strength(player_id, line_number, counter_example)

    def validate_counter_example_strength(self, player_id, line_number, counter_example):
        '''Checks that the counter_example is strictly stronger than <other_player> side of that line '''
        other_player_id = self.get_other_player(player_id)
        other_hand = self.game.lines[line_number].sides[other_player_id]
        other_hand = [c for c in other_hand if c in JOKERS + NUMBERS_CARDS]
        is_foggy = self.game.lines[line_number].contains("FOG")
    ...
    def validate_counter_example_format(self, player_id, line_number, counter_example):
        '''checks that the counter example is possible, given the available public information'''
        other_player_id = self.get_other_player(player_id)
        played_cards = self.game.lines[line_number].sides[player_id]
    ...
    def get_used_cards(self):
        ...
        result = public_cards + hands
        assert len(result) == len(set(result)), 'We expect no double occurrences of cards: ' + str([x for x in result if result.count(x) > 1])
        return result
    ...
    def play_card(self, player_id, line_number, card, other_card=None):
        ...
        self.validate_played_card(player_id, line_number, card, other_card)
        if card in ["REDEPLOY", "TRAITOR"]:
            self.move_card_to(other_card, player_id, line_number)
        elif card == "DESERTER":
            self.desert_card(other_card)
        g  = self.game
        g.hands[player_id].remove(card)
        self.add_card_to_line(player_id, line_number, card)
    ...
    def move_card_to(self, card, player_id, line_number):
        ...
        self.game.lines[line_number].sides[player_id].append(card)

    def add_card_to_line(self, player, line, card):
        self.game.lines[line].sides[player].append(card)


    def validate_played_card(self, player_id, line_number, card, other_card):
        '''Checks that
